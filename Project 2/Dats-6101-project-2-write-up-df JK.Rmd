...
Building upon exploratory data analysis, we researched what factors could effect teenage pregnancy. Originally, we only looked at if the person was likely to have an abstinence only education on the likelihood of them getting pregnant. From our last segment, we realized we had too few observations and too few co-variates.

# Data Set Description 

Using our findings from the first project, we changed our dataset to the Guttmacher Institute's report: "Pregnancies, Births and Abortions in the United States, 1973–2017: National and State Trends by Age." This dataset uses  publicly available CDC data to estimate the number of births and distribution of abortions by age. The population of women came from Census Bureau data while the raw number of abortions came from internal Guttmacher Institute data (Maddow-Zimet & Kost, 2021). 
...
## Cleaning Dataset
We split the data into two data sets: one including data for all states, and another one containing data for the United States as a whole.
```{r, results='hide'}
...
str(p2US)
```
Secondly, we converted the abstinence, abortionban_anomaly, abortionban_sex, and substance_poli variables into factor variables.
```{r}
# Factor Conversions
...
The histogram (figure 2) and boxplot (figure 3) indicate that it is slightly right-skewed, likely due to the presence of outliers. However, the QQ-plot (figure 4) closely follows the normal line, suggesting that the distribution is still considered to be normal.

While we could remove all outliers to eliminate their influence, we need to carefully consider why they are there in the first place. This could impact the validity of the results.
...
### Comparing Covering Abstinence and Not Covering Abstinence
